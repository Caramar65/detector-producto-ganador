...
const input=req.body?.products;if(!Array.isArray(input)||input.length!==5)return res.status(400).json({error:"La V3.1 necesita exactamente 5 productos."});
...
products:{type:"array",minItems:5,maxItems:5,items:{type:"object",additionalProperties:false,properties:{
...
const prompt=`Compara estos 5 productos para ecommerce/dropshipping en Colombia. Usa investigación web actual y verificable cuando sea posible. No inventes ventas ni cifras. Si una señal no es verificable, declárala como estimación.
...
let text=apiResult.output_text||"";if(!text)return res.status(502).json({error:"OpenAI no devolvió contenido."});
