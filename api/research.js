module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const researchVersion = "V3.8";

    const block = products.map(p => `PRODUCTO ${p.id}: ${p.productName}\nDescripción: ${p.description || "No proporcionada"}\nURL PROPORCIONADA: ${p.url || "No proporcionada"}\nCosto ${p.cost} COP | Envío ${p.shipping} | Otros ${p.otherCosts} | Venta ${p.salePrice} | Devoluciones ${p.returns}%\nMargen servidor ${p.margin.toFixed(0)} COP (${p.marginPercent.toFixed(2)}%) | CPA máx ${p.maxCPA.toFixed(0)} | CPA objetivo ${p.targetCPA.toFixed(0)} | ROAS equilibrio ${p.breakEvenROAS.toFixed(2)}`).join("\n\n");

    const short = { type: "string", maxLength: 500 };
    const tiny = { type: "string", maxLength: 280 };
    const smallList = { type: "array", maxItems: 2, items: tiny };
    const evidenceItem = {
      ...
      properties: {
        ...
        evidence: short,
        sourceUrls: { type: "array", maxItems: 3, items: { type: "string", maxLength: 500 } }
      },
      required: ["claim", "type", "evidence", "sourceUrls"]
    };
    const sourceItem = {
      type: "object", additionalProperties: false,
      properties: { title: tiny, url: { type: "string", maxLength: 500 }, type: tiny, supports: short, note: short },
    ...
    const productSchema = {
      ...
      properties: {
        ...
        strengths:smallList, weaknesses:smallList, risks:smallList, angles:smallList, testPlan:smallList,
        evidence:{type:"array",minItems:2,maxItems:3,items:evidenceItem}
      },
    ...
    const schema = {
      ...
      properties:{
        ...
        recommendation:short,researchNotes:{type:"array",maxItems:10,items:short},sources:{type:"array",maxItems:12,items:sourceItem}
      },
      required:["products","overallWinner","metaWinner","tiktokWinner","recommendation","researchNotes","sources"]
    };

    const prompt = `Analiza ecommerce y publicidad digital para Colombia. Compara ${products.length} producto(s) y decide ganador general, Meta Ads y TikTok Ads.

WEB: obligatoria pero rápida. Máximo 2 búsquedas por producto. Prioriza una fuente oficial/regulatoria si aplica y una fuente de mercado/oferta. No hagas búsquedas redundantes. La URL del usuario sirve para verificar producto, precio o características, no para demostrar demanda.

TRAZABILIDAD: no inventes URLs, ventas, CTR, CPA históricos, ROAS históricos, volúmenes ni cifras de mercado. Cada producto debe tener 2-3 evidencias breves clasificadas como verified, inference o recommendation. sourceUrls solo URLs realmente encontradas o proporcionadas por el usuario. sources debe usar esas mismas URLs.

CRITERIOS: demand, competitionOpportunity, visual, differentiation e impulse de 0-5. economicScore, metaScore, tiktokScore y overallScore de 0-100. Ponderación general: demanda 20%, competencia 15%, visual 15%, diferenciación 10%, impulso 10%, economía 30%. 80+ prioritario; 70-79 testear; 60-69 precaución; 50-59 débil; <50 no prioritario.

ECONOMÍA: usa exactamente los valores del servidor para margen, marginPercent, maxCPA, targetCPA y breakEvenROAS.

SALUD: evita afirmaciones clínicas no verificadas. Usa lenguaje de bienestar y cumplimiento. Fortalezas, debilidades, riesgos, ángulos y plan: máximo 2 puntos compactos cada uno.

PRODUCTOS:\n${block}\n\nDevuelve únicamente JSON válido según el esquema. Sin markdown y sin texto adicional.`;
    ...
    try {
      response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body:JSON.stringify({
          ...
          text:{verbosity:"low",format:{type:"json_schema",name:"product_comparison_traceable_v38",strict:true,schema}}
        }),
        signal:controller.signal
      });
    } catch (fetchError) {
      if (fetchError?.name === "AbortError") return res.status(504).json({error:"La investigación superó el tiempo disponible. Intenta nuevamente; la consulta fue optimizada para 1–5 productos.",researchId,researchVersion});
      throw fetchError;
    ...
    if (apiData?.incomplete_details?.reason === "max_output_tokens") {
      console.error("OpenAI output truncated by max_output_tokens", researchId);
      return res.status(502).json({error:"La investigación produjo una respuesta demasiado extensa. La consulta fue optimizada; vuelve a intentarlo.",researchId,researchVersion});
    }
    ...
    const walk=value=>{if(!value||typeof value!=="object")return;if(Array.isArray(value)){value.forEach(walk);return;}if(value.type==="url_citation"||value.type==="url_citation_detail")addCitation(value.url,value.title||value.name||"");for(const child of Object.values(value))walk(child);};
    walk(apiData.output);
    ...
    try { result=JSON.parse(text); }
      ...
      console.error("JSON parse error:",parseError); console.error("OpenAI text:",text.slice(0,5000));
    ...
    const userSources=products.filter(p=>p.url&&/^https?:\/\//i.test(p.url)).map(p=>({title:`${p.productName} — URL proporcionada por el usuario`,url:p.url,type:"user_provided",supports:"Página proporcionada para el análisis del producto.",note:"Fuente proporcionada por el usuario; verifica producto, precio o características, pero no demuestra por sí sola la demanda."}));
    ...
    result.products=result.products.map(p=>{
      const evidence=(Array.isArray(p.evidence)?p.evidence:[]).map(e=>{
        ...
        if(clean.claim&&clean.evidence){const titles=sourceUrls.map(u=>sourceMap.get(u)?.title||u);evidenceNotes.push(`${p.productName} — ${typeLabel[type]}: ${clean.claim}. ${clean.evidence}.${titles.length?` Fuente: ${titles.join("; ")}.`:""}`);}
        return clean;
      });
      return {...p,evidence};
    });
    const existingNotes=Array.isArray(result.researchNotes)?result.researchNotes.map(x=>String(x).trim()).filter(Boolean):[];
    result.researchNotes=[...evidenceNotes,...existingNotes].slice(0,20);
